 ...
 */
function sync (hub, state, {seed, security = 2, rescan = true}) {
  if (!seed) {
  ...
  return (rescan ? syncAddresses(hub, stateCopy, seed, security) : Promise.resolve(stateCopy))
    ...
    .then(state => getTransfersDiff(hub, state, rescan)
      ...
      .then(diff => applyTransfersDiff(hub, state, diff))
    )

    // Update the list of inputs
    .then(state => categorizeToInputs(state))
}
...
function syncAddresses (hub, state, seed, security = 2) {
  // Get next key index
  const index = state.lastKeyIndex + 1
  ...
  return hub.api.getNewAddress(seed, {index, security, returnAll: true, checksum: false})
    ...
    .then(newAddresses => {
      ...
      state.addresses.concat(newAddresses.map((address, i) => ({address, index: index + i, security})))

      // Update state with new last key index
      state.lastKeyIndex = newAddresses[newAddresses.length - 1].index

      // Return the synchronized state copy
      return state
    })
}
 ...
 */
function applyTransfersDiff (hub, state, diff) {
  const diffCopy = [...diff].filter(tx => tx.value <= 0) // Diff copy used to validate the signatures
  const seenSpentAddresses = new Set() // Set of seen spent addresses used for skipping extra validationss
  const sweeps = new Set() // Set of sweep bundle hashes

  for (const tx of diff) {
    if (tx.value < 0) { // Process spending transactions
    ...
    } else if (tx.value > 0 && tx.persistence) { // Process positive value transactions
      ...
      if (bundle.length - 1 === tx.lastIndex && !sweeps.has(tx.bundle)) {
        ...
        if (utils.isBundle(bundle) && !isSendingValueToInputs(bundle)) {
          ...
          // Emit sweep event
          hub.emit('sweep', bundle)
        }
      } else if (bundle.findIndex(tx => tx.value < 0) === -1) { // Handle deposit transactions
        hub.emit('deposit', tx)
      }
    }
...
function categorizeToInputs (state) {
  // Find addresses that need to be moved to inputs list
  const inputs = state.addresses.filter(address => address.balance > 0 && !isUsedKey(state.indexes, address.index))

  // Move each input address to inputs list
...
function isSendingValueToInputs (bundle) {
  ...
  return inputTxs.every(inputTx => outputTxs.some(outputTx => outputTx.address === inputTx.address))
}
