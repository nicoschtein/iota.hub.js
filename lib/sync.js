...
function getTransfersDiff (hub, state, previousState, rescan = true) {
  ...
  if (rescan) {
    ...
    addresses = currentStateAddresses.filter(b => previousStateAddresses.find(a => a.address === b.address).balance !== b.balance)
  }
  ...
  return hub.api.findTransactionObjects({ addresses: addresses.map(address => address.address) })
    ...
    .then(txs => txs.filter(tx => state.transfers.findIndex(stateTx => stateTx.hash === tx.hash && stateTx.persistence) === -1))

    // Include all unconfirmed transfers
    .then(txs => txs.concat(state.transfers.filter(stateTx => stateTx.hash && !stateTx.persistence && txs.findIndex(tx => tx.hash === stateTx.hash) === -1)))
    ...
    .then(txs => txs.filter(tx => state.transfers.findIndex(stateTx => stateTx.hash === tx.hash && stateTx.persistence !== tx.persistenece) === -1))
}
...
function applyTransfersDiff (state, diff, eventEmitter) {
  ...
  if (eventEmitter) {
    ...
    for (const tx of diff.filter(tx => tx.value > 0 && tx.persistence)) {
      ...
      const bundle = [...diff].filter(diffTx => diffTx.bundle === tx.bundle && diffTx.persistence).sort((a, b) => a.currentIndex - b.currentIndex)
