  ...
  for (const transfer of transfers) {
    ...
    for (let i = 0; i < fragmentsLength; i++) {
      signatureMessageFragments.push(
        pad(getFragment(transfer.message || '', i, SIGNATURE_MESSAGE_FRAGMENT_LENGTH), SIGNATURE_MESSAGE_FRAGMENT_LENGTH)
      )
    }
  }
    ...
    for (let j = 0; j < input.security; j++) {
      bundle.bundle[offset++].signatureMessageFragment = converter.trytes(
        signing.signatureFragment(
          getFragment(normalizedBundleHash, j, HASH_LENGTH / 3),
          getFragment(key, j, KEY_FRAGMENT_LENGTH)
        )
      )
    }
  }
